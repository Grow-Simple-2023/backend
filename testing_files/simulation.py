...
def item_status_update(item_id, status, rider_id):
    url = f'http://localhost:8000/rider/item-status-update/{rider_id}/'
    data = {'item_id': item_id, 'status': status,'OTP':0}
    r = requests.post(url, json=data)
    return r

def update_rider_location(rider_id,lat_long):
    url = f'http://localhost:8000/rider/send-self-location/{rider_id}/'
...
def get_all_routes():
    url = 'http://localhost:8000/manager/items-in-delivery/'
    r = requests.get(url)
    res = r.json()
    print(res)
